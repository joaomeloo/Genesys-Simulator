...
extern "C" void onDispatchEvent(Simulator* simulator, Model* model, Entity* entity) {
	try {
        ...
        model->getTracer()->traceError("Error while executing onDispatchEvent cpp user code in CppForG_1.", e);
	}
}
...
extern "C" void initBetweenReplications(Model* model) {
	try {
        ...
        model->getTracer()->traceError("Error while executing initBetweenReplications cpp user code in CppForG_1.", e);
	}
}
