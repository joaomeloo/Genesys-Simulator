...
void ODESolver::_createEditableDataDefinitions() {
	for (const std::string& variableName : _stateVariableNames) {
		_mandatoryEditableDataDefinitionInsert("Variable", _parentModel->getDataManager()->getDataDefinition(Util::TypeOf<Variable>(), variableName));
	}
}
...
std::vector<double> ODESolver::_loadStateVector(PersistenceRecord* fields, const std::string& prefix) const {
	...
	for (unsigned int i = 0; i < _stateVariableNames.size(); i++) {
		const std::string fieldName = prefix + std::to_string(i);
		values.push_back(fields->loadField(fieldName, 0.0));
	}
	return values;
}
...
void ODESolver::_saveStateVector(PersistenceRecord* fields,
	...
	for (unsigned int i = 0; i < values.size(); i++) {
		const std::string fieldName = prefix + std::to_string(i);
		fields->saveField(fieldName, values[i], 0.0, saveDefaultValues);
	}
}

double ODESolver::_readVariableValue(const std::string& variableName) const {
	Variable* var = dynamic_cast<Variable*>(_parentModel->getDataManager()->getDataDefinition(Util::TypeOf<Variable>(), variableName));
	return var != nullptr ? var->getValue() : 0.0;
}
...
void ODESolver::writeStateToVariables() const {
	for (unsigned int i = 0; i < _stateVariableNames.size() && i < _stateValues.size(); i++) {
		Variable* var = dynamic_cast<Variable*>(_parentModel->getDataManager()->getDataDefinition(Util::TypeOf<Variable>(), _stateVariableNames[i]));
		if (var != nullptr) {
			var->setValue(_stateValues[i]);
		}
	}
	Variable* timeVar = dynamic_cast<Variable*>(_parentModel->getDataManager()->getDataDefinition(Util::TypeOf<Variable>(), _timeVariableName));
	if (timeVar != nullptr) {
